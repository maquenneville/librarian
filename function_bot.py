...
# =============================================================================
        ...
        while True:
            try:
                ...
                if function_call:  # Check if function_call is present
                    ...
                    print(f'Using {function_name}...')
                    result = self._truncate_function_response(self._execute_function(function_name, function_args))
                    
                    function_message = {"role": "function", "content": f"Finished executing function {function_name}, Args: {function_args}. Result: {result}", 'name': function_name}
                ...
                messages.append({"role": "function", "content": f"Finished executing function {function_name}. Result: {e}", 'name': function_name})
        return response, function_messages
    
    def _truncate_function_response(self, func_response):
        
        func_string = str(func_response)
        
        num_tokens = self.count_tokens(func_string)
        
        allowed_text = ""
        
        if num_tokens > 125000:
            res_tokens = 0
            text_chunks = func_string.split(" ")
            
            while res_tokens < 122000:
                chunk = text_chunks.pop(0)
                
                allowed_text += chunk
                
                tokes = self.count_tokens(allowed_text)
                
                res_tokens += tokes
                
        else:
            allowed_text = func_string
    ...
    def chat(self, input_string: str):
        ...
        response, func_mess = self._generate_function_response(self.messages, temperature=0.1, function_desc=self.function_desc)
        
        if response is not None:
            self.messages.extend(func_mess)
